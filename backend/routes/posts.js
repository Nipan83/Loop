...
function autoCategorize(title, content) {
  ...
  return category
    ? category.id
    : db.prepare("SELECT id FROM categories WHERE slug = ?").get("general").id;
}
...
router.get("/", optionalAuth, (req, res) => {
  try {
    ...
    // If user is logged in, check their upvotes, bookmarks, and follows
    if (req.user) {
      const userUpvotes = db
        .prepare("SELECT post_id FROM post_upvotes WHERE user_id = ?")
        .all(req.user.id);
      const userBookmarks = db
        .prepare("SELECT post_id FROM bookmarks WHERE user_id = ?")
        .all(req.user.id);
      const userFollows = db
      ...
      const followedUsers = new Set(userFollows.map((f) => f.following_id));

      posts.forEach((post) => {
        post.hasUpvoted = upvotedPosts.has(post.id);
        post.isBookmarked = bookmarkedPosts.has(post.id);
        post.isFollowing = followedUsers.has(post.user_id);
      });
    }
...
router.get("/:id", optionalAuth, (req, res) => {
  try {
    ...
    if (req.user) {
      const postUpvote = db
        .prepare(
          "SELECT id FROM post_upvotes WHERE user_id = ? AND post_id = ?",
        )
      ...
      const replyUpvotes = db
        .prepare("SELECT reply_id FROM reply_upvotes WHERE user_id = ?")
        .all(req.user.id);
...
router.post("/:id/upvote", requireAuth, (req, res) => {
  try {
    ...
    const post = db
      .prepare("SELECT id, upvotes FROM posts WHERE id = ?")
      .get(postId);
    if (!post) {
      return res.status(404).json({ error: "Post not found" });
    }
    ...
    const existingUpvote = db
      .prepare("SELECT id FROM post_upvotes WHERE user_id = ? AND post_id = ?")
      .get(userId, postId);
    ...
    if (existingUpvote) {
      ...
      const updatedPost = db
        .prepare("SELECT upvotes FROM posts WHERE id = ?")
        .get(postId);
